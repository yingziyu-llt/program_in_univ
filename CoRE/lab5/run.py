...
from bc_utils import compute_bc_loss, create_actor_network, normalize_observations
from env_utils import evaluate, make_eval_envs
...
class Args:
    ...
    demo_path: str = "~/.maniskill/demos/PushCube-v1/motionplanning/trajectory.state.pd_ee_delta_pos.physx_cpu.h5"
...
class ManiSkillDataset(Dataset):
    def __init__(
        ...
        # For details on how the code below works, see the quick start tutorial
        ...
        if normalize_states:
            # Use student's normalization function
            self.observations, self.obs_mean, self.obs_std = normalize_observations(self.observations)
...
class Actor(nn.Module):
    def __init__(self, state_dim: int, action_dim: int):
        ...
        # Use student's designed architecture
        self.net = create_actor_network(state_dim, action_dim)
...
def main():
    ...
    for iteration, batch in enumerate(dataloader):
        ...
        optimizer.zero_grad()
        # Use student's loss function
        loss = compute_bc_loss(pred_action, action)
        loss.backward()
        optimizer.step()
