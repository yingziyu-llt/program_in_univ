...
class FeatureAgent(MahjongGBAgent):
    ...
    OBS_SIZE = 22  # 2 + 4 * 5 门+手牌+四人打出去的牌
    ACT_SIZE = 235
    ...
    def _hand_embedding_update(self):
        ...
        dd = [defaultdict(int)] * 4
        for index, tiles in enumerate(self.history):
            for tile in tiles:
                dd[index][tile] += 1
        for index, ddd in enumerate(dd):
            for tile in ddd:
                self.obs[
                    self.OFFSET_OBS["MAN" + str(index + 1)] : self.OFFSET_OBS[
                        "MAN" + str(index + 1)
                    ]
                    + dd[index][tile],
                    self.OFFSET_TILE[tile],
        ...
        for index, ddd in enumerate(dd):
            for tile in ddd:
                ...
                if dd[index][tile] == 3:
                    self.obs[
                        self.OFFSET_OBS["MAN" + str(index + 1)] + 3,
                        self.OFFSET_TILE[tile],
                    ] += 3
                    self.obs[
                        self.OFFSET_OBS["MAN" + str(index + 1)] + 3,
                ...
                if dd[index][tile] == 2:
                    self.obs[
                        self.OFFSET_OBS["MAN" + str(index + 1)] + 1 : self.OFFSET_OBS[
                            "MAN" + str(index + 1)
                        ]
                        + 3,
                        self.OFFSET_TILE[tile],
