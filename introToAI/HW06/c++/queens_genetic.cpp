...
public:
    inline std::vector< ChromosomeType > population() const override {
        return _population;
    }
    inline std::vector< double > adaptability() const override {
    ...
    void mutate() override {
        ...
        for ( int i = 0; i < _population.size(); ++i ) {
            if ( RandomVariables::uniform_real() > mutation_rate / _adaptability[ i ] * n * ( 1 - n ) + 0.15 ) {
                continue;
            }
            new_c = _population[ i ];
            mutate( new_c );
    ...
    QueensPopulation( const std::vector< ChromosomeType >& p, double _mutation_rate )
        : _population( p ), mutation_rate( _mutation_rate ), _adaptability( p.size() ), best_index( 0 ), second_index( 0 ) {
        update_adaptability();
    }
};
