...
template < typename StateLocalType > class SimulatedAnneal {
private:
    ...
    const StateLocalType& sample_path( ValueEstimatorType value_of, TemperatureScheduleType temperature_at ) {
        ...
        for ( int t = 0; state.neighbor_count() > 0 and temperature >= terminate_temperature; ++t ) {
            ...
            if ( value_diff > 0.0 || RandomVariables::uniform_real() < exp( value_diff / temperature ) ) {
                state = new_state;
            }
        }
...
public:
    ...
    bool search( ValueEstimatorType value_of, TemperatureScheduleType temperature_at, int iterations, double target_value, double _terminate_temperature = 1e-10 ) {
