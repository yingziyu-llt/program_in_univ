...
template < typename GameState > class MonteCarloTreeSearch {
private:
    ...
    std::vector< double > sample_path( const GameState& state, double exploration ) {
        ...
        if ( node->n_children() < state.n_actions() ) {
            ...
            for ( int i = 0, child; i < node->n_children(); ++i ) {
                ...
                selection.submit( value_sums_of[ node->index() ][ child ] / visit_count_of[ child ] + exploration * sqrt( 2 * log( visit_count_of[ node->index() ] ) / visit_count_of[ child ] ) );
            }
