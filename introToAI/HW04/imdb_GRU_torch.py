...
class ImdbNet(Module):
    def __init__(self):
        ...
        self.gru = GRU(input_size=64, hidden_size=64, batch_first=False)
        self.linear1 = Linear(in_features=64, out_features=128)
        self.act1 = torch.nn.ReLU()
        self.linear2 = Linear(in_features=128, out_features=2)
...
# 实例化训练集和网络
writer = SummaryWriter("./imdb_lstm_torch.log")
...
def train(model, device, train_loader, optimizer, epoch):
    ...
    return train_loss / n_iter, train_acc / n_iter
...
for epoch in range(1, n_epoch + 1):
    train_loss, train_acc = train(net, device, train_loader, optimizer, epoch)
    test_loss, test_acc = test(net, device, test_loader)
    writer.add_scalars(
        "raw_lstm/losses", {"train_loss": train_loss, "test_loss": test_loss}, epoch
    )
    writer.add_scalars(
        "raw_lstm/acc", {"train_acc": train_acc, "test_acc": test_acc}, epoch
    )
