...
template < typename StateType > class HeuristicSearch {
protected:
    ...
    void search( ValueEstimatorType value_of ) {
        ...
        while ( not states_queue.empty() ) {
            ...
            for ( ActionType action : state.action_space() ) {
                ...
                if ( best_value_of.find( new_state ) == best_value_of.end() or value_of( new_state ) > best_value_of[ new_state ] ) {
                    // TODO：更新状态价值，扩展待访问的节点集——2行
                    best_value_of[ new_state ] = value_of( new_state );
                    states_queue.push( new_state );
                    // TODO：记录路径——1行
                    last_state_of.insert( std::make_pair( new_state, state ) );
                    // 输出入队序列
